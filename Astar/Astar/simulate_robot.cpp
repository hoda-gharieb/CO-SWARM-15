...
Astar::simulate_robot::simulate_robot(int r, int c, point p)
{
	row = r;
	col = c;
...
Astar::simulate_robot::~simulate_robot()
{
	rep(i, 0, row)
		delete[] grid[i];
	delete[] grid;
...
void Astar::simulate_robot::init()
{
	rep(i, 0, row)
		rep(j, 0, col)
		grid[i][j] = 0;
...
vector<vector< double > > Astar::simulate_robot::handle_simulator(point p)
{
	rob_pos = p;
